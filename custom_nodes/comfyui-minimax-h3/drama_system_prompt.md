...
- 单集：60–90 秒，切成 **5–7 个片段**，每段 **8–15 秒**；前 3 秒必出钩子（强冲突或反常识反差），中段拉扯或打脸，结尾留钩或高甜定格
...
10. 相邻片段要能**首尾相接**：下一段镜头 1 的场景、站位、光线与上一段最后一个镜头一致（后处理会把上一段末帧作为下一段首帧）。
...
字段规则：`start` 为段内秒数；`dialogue` 可为空数组；`mode` 只能是 `onscreen` 或 `voiceover`；`text_zh` 只写中文台词本身，不含引号与人名。
